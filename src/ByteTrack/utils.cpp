...
namespace ByteTrack
{
    ...
    {
        std::map<std::uint64_t, int> exists;
        std::vector<STrack *> res;
        for (int i = 0; i < tlista.size(); i++)
        {
            exists.insert(std::pair<std::uint64_t, int>(tlista[i]->track_id, 1));
            res.push_back(tlista[i]);
        }
        for (int i = 0; i < tlistb.size(); i++)
        {
            std::uint64_t tid = tlistb[i].track_id;
    ...
    {
        std::map<std::uint64_t, STrack> stracks;
        for (int i = 0; i < tlista.size(); i++)
        {
            stracks.insert(std::pair<std::uint64_t, STrack>(tlista[i].track_id, tlista[i]));
        }
        for (int i = 0; i < tlistb.size(); i++)
        {
            std::uint64_t tid = tlistb[i].track_id;
        ...
        std::vector<STrack> res;
        std::map<std::uint64_t, STrack>::iterator it;
